...
def split_data(data, target, feature_index, split_value):
    # Two boolean arrays indicating whether the index should go to the left or right subtree, 
...
def split_node(data, target, criterion):
    ...
    # Considering all possible split points ordered in ascending value
    for feature_index in range(data.shape[1]):
        unique_values = np.unique(data[:, feature_index])
        # Each split point is an average of two nearest unique feature values
        ...
        for split_value in split_points:
            ...
            if len(left_target) != 0 and len(right_target) != 0:
                ...
                # Performing the first encountered split decreasing the criterion the most
...
def make_node_non_recursive(data, target, max_depth, max_leaves, min_to_split, criterion):
    ...
    # Splitting if:
    #  - 'args.max_depth` is not `None`, its depth must be less than `args.max_depth`
    #  - `args.max_leaves` is not `None`, there are less than `args.max_leaves` leaves
...
def main(args: argparse.Namespace) -> tuple[float, float]:
    ...
    # Manually create a decision tree on the training data.
    tree = decision_tree(
